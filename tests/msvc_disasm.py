...
def relocation_target(
    ...
    if symbol.section is not None:
        ...
        if symbol.section.name == ".text":
            ...
            labels.add(target_offset)
            return address_label(target_offset)
...
def collect_relocation_labels(section: CoffSection, labels: Set[int]) -> None:
    for reloc in section.relocations.values():
        if reloc.symbol.section is not None and reloc.symbol.section.name == ".text":
            target_offset = reloc.symbol.offset + reloc.symbol_offset
            if target_offset not in reloc.symbol.section.symbols:
                labels.add(target_offset)


def disassemble_text_section(section: CoffSection, output: TextIO) -> None:
    ...
    for insn, _addr, _raw in disassembly:
        if insn.mnemonic.startswith("j") and insn.operands and insn.imm_size:
            target = insn.operands[-1].imm
            if target not in section.symbols:
                labels.add(target)
...
def disassemble_msvc_coff(coff_in: BinaryIO, asm_out: TextIO) -> None:
    ...
    for section in coff.sections.values():
        if not should_emit_section(section):
            continue
        asm_out.write(f".section {section.name}\n")
        if section.name == ".text":
            disassemble_text_section(section, asm_out)
        else:
