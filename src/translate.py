...
class LocalVar:
    value: int = attr.ib()
    type: Type = attr.ib(eq=False, order=False)
...
class PassedInArg:
    value: int = attr.ib()
    copied: bool = attr.ib(eq=False, order=False)
    stack_info: StackInfo = attr.ib(eq=False, order=False)
    type: Type = attr.ib(eq=False, order=False)
...
class StructAccess:
    ...
    # This has eq=True, order=True since it represents a live expression and not
    ...
    field_name: Optional[str] = attr.ib(eq=False, order=False)
    stack_info: StackInfo = attr.ib(eq=False, order=False, repr=False)
    type: Type = attr.ib(eq=False, order=False)
...
@attr.s(frozen=True, eq=True, order=True)
class ArrayAccess:
    # Represents ptr[index]. eq=True, order=True for symmetry with StructAccess.
...
class Literal:
    value: int = attr.ib()
    type: Type = attr.ib(eq=False, order=False, factory=Type.any)
...
class AddressOf:
    expr: "Expression" = attr.ib()
    type: Type = attr.ib(eq=False, order=False, factory=Type.ptr)
...
@attr.s(frozen=False, eq=False, order=False)
class EvalOnceExpr:
