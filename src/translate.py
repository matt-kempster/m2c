...
def assign_phis(used_phis: List[PhiExpr], stack_info: StackInfo) -> None:
    ...
    for phi in used_phis:
        if not phi.replacement_expr and phi.propagates_to() == phi:
            ...
            prefix = f"phi_{stack_info.function.used_reg_names.internal_to_output(phi.reg).register_name}"
            ...
            if not data.meta.force and expr_filter(expr):
                ...
                if not isinstance(expr, EvalOnceExpr):
                    expr = eval_once(
                        ...
                        trivial=False,
                        prefix=stack_info.function.used_reg_names.internal_to_output(r).register_name,
                    )
    ...
    def set_reg(reg: Register, expr: Optional[Expression]) -> Optional[Expression]:
        ...
        if not isinstance(expr, Literal):
            expr = eval_once(
                ...
                prefix=stack_info.function.used_reg_names.internal_to_output(reg).register_name,
            )
            ...
            for out in instr.outputs:
                ...
                if not isinstance(val, SecondF64Half):
                    val = eval_once(
                        ...
                        emit_exactly_once=False,
                        trivial=False,
                        prefix=stack_info.function.used_reg_names.internal_to_output(out).register_name,
                    )
