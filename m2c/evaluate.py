...
def replace_xor_shift(expr: BinaryOp) -> BinaryOp:
    """
    Simplify the expressions matching `(((b ^ a) >> 1) - ((b ^ a) & b)) >> 31`
    into `a < b` or `(((b ^ a) >> 1) - ((b ^ a) & a)) >> 31` into `b < a`,
    generated code wise the direction of the operator doesn't matter.
    These are generated by GW MW 2.6 for example.
    """
    if expr.is_floating() or expr.op != ">>":
        return expr
    ...
    sub_left_expr = early_unwrap(left_expr.left)
    sub_right_expr = early_unwrap(left_expr.right)
    if not (
    ...
    sub_left_xor_expr = early_unwrap_ints(sub_left_expr.left)
    sub_right_xor_expr = early_unwrap_ints(sub_right_expr.left)

    if not (
        isinstance(sub_left_xor_expr, BinaryOp)
        and sub_left_xor_expr.op == "^"
        and sub_left_xor_expr == sub_right_xor_expr
    ):
        return expr

    sub_right_right_expr = early_unwrap(sub_right_expr.right)

    if early_unwrap(sub_left_xor_expr.left) == sub_right_right_expr:
        # (b ^ a) ... & b
        return BinaryOp.icmp(sub_left_xor_expr.right, "<", sub_left_xor_expr.left)
    elif early_unwrap(sub_left_xor_expr.right) == sub_right_right_expr:
        # (b ^ a) ... & a
        return BinaryOp.icmp(sub_left_xor_expr.left, "<", sub_left_xor_expr.right)
...
def replace_shift_add_carry(expr: BinaryOp) -> BinaryOp:
    ...
    into `a <= b`, generated code wise the direction of the operator doesn't matter.
    These are generated by GW MW 2.6 for example.
    """

    if expr.is_floating() or not (expr.op == "+" and isinstance(expr.right, CarryBit)):
        return expr
    ...
    return BinaryOp.icmp(left_right_expr.left, "<=", left_left_expr.left)
...
def carry_add_to(expr: Expression) -> BinaryOp:
    carry_expr = BinaryOp.intptr(expr, "+", CarryBit())
    carry_expr = fold_divmod(carry_expr)
    carry_expr = replace_shift_add_carry(carry_expr)
    return carry_expr
